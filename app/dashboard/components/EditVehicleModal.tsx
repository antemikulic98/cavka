...
export default function EditVehicleModal({
  ...
  // Initialize form data with vehicle data when modal opens
  useEffect(() => {
    if (isOpen && vehicle) {
      setFormData({
        make: vehicle.make,
        ...
        fuelAirCon: vehicle.fuelAirCon || 'R',
        passengerCapacity: vehicle.passengerCapacity,
        doorCount: vehicle.doorCount,
        bigSuitcases: vehicle.bigSuitcases || 2,
  ...
  const handleUpdateVehicle = async () => {
    if (currentStep !== 4 || loading || uploadingImages) {
      return;
    }
    ...
    try {
      ...
      setValidationErrors([`Failed to update vehicle: ${errorMessage}`]);
                ...
                <div className='mt-4'>
                  <div className='flex items-center justify-between mb-2'>
                    <p className='text-sm font-medium text-gray-900'>
                      New files to upload: {selectedImages.length}
                    </p>
                    {uploadingImages && (
          ...
          <div className='bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 modal-content'>
            <div className='flex justify-between items-center mb-6'>
              <h2 className='text-xl font-bold text-gray-900 flex items-center'>
                <Edit className='h-6 w-6 mr-3 text-emerald-600' />
                Edit Vehicle
              </h2>
              <button
                onClick={onClose}
