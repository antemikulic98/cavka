...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Validate rentalDays is a positive number
    if (typeof bookingData.rentalDays !== 'number' || bookingData.rentalDays < 1) {
      return NextResponse.json(
    ...
    // The day count is derived from the dates — never from the client value.
    const { calculateBookingPrice, computeRentalDays } = await import(
      '@/lib/pricing'
    );
    ...
    // Create booking (bookingReference is auto-generated by the Booking model pre-save hook)
    const booking = new Booking({
      ...
      rentalDays,
      cdwCoverage: bookingData.cdwCoverage || 'none',
      addOns: bookingData.addOns || {},
      paymentMethod: bookingData.paymentMethod || 'pay_later',
      pricing: {
        baseDailyRate:
          Math.round((pricing.baseVehicleCost / rentalDays) * 100) / 100,
        cdwCost: pricing.cdwCost,
        addOnsCost: pricing.addOnsCost,
        totalDailyRate:
          Math.round((pricing.totalBeforeDiscount / rentalDays) * 100) / 100,
        totalCost: Math.round(pricing.totalAfterDiscount * 100) / 100,
        discount: Math.round(pricing.discountAmount * 100) / 100,
        originalAmount: Math.round(pricing.totalBeforeDiscount * 100) / 100,
      },
      // Overbooking fields
      isOverbooking,
